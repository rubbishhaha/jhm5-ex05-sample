...
// .wrangler/tmp/bundle-9C83QC/strip-cf-connecting-ip-header.js
...
async function handleRequest(request, env, ctx) {
  ...
  if (path === "/api/hkdse/stats" && request.method === "GET") {
    try {
      if (!env.DB)
        return jsonResponse({ error: "no d1 binding" }, 500);
      const res = await env.DB.prepare("SELECT 1 AS ok").all();
      return jsonResponse({ rows: res.results || res });
    } catch (e) {
...
if (typeof addEventListener === "function") {
  addEventListener("fetch", (event) => {
    try {
      event.respondWith(handleRequest(event.request, event.env || event.bindings || {}, event));
    } catch (e) {
...
// .wrangler/tmp/bundle-9C83QC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-9C83QC/middleware-loader.entry.ts
